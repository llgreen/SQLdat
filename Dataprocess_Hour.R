...
setwd("C:/Users/FengMac/Desktop/workspace")
...
Timeperiod <- paste("2016-10-18 ", hour, ":00:00",  sep = "")
...
Timeahead <- 2 #  ( 1 - 24) # how long (2 - 10 hours) before the product you want to consider
...
db_contactID <- dbGetQuery(conn_comxerv, paste("SELECT *, from_unixtime(deliveryStart/1000) as FmDate,", 
                                               ...
                                               "WHERE deliveryStart =", product, "and duration = 1 ORDER BY deliveryStart;"))
...
tmp <- as.numeric(substr(Timeperiod, 12,13))+1
...
figurename <-paste(substr(Timeperiod, 1,13), "-", tmp, " (",quantCont,"MW).png", sep = "")
...
title(main = paste(substr(Timeperiod, 1,16), "-", tmp, ":00",  " (",quantCont,"MW)", sep = ""))
